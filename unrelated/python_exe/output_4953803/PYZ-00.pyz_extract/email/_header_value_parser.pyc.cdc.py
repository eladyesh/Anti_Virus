...
class BareQuotedString(QuotedString):
    ...
    def __str__(self):
        return quote_string(''.join((lambda .0: pass# WARNING: Decompyle incomplete
)(self)))
...
class Terminal(str):
    ...
    def _pp(self = None, indent = None):
        if not self.defects:
            return [
                '{}{}/{}({}){}'.format(indent, self.__class__.__name__, self.token_type, super().__repr__(), '')]
        return [
            None('{}{}/{}({}){}'.format, indent, self.__class__.__name__, self.token_type, super().__repr__().format(self.defects))]
