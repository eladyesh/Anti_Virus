...
def Fraction():
    ...
    def __new__(cls = None, numerator = None, denominator = None, *, _normalize):
        ...
        if denominator is None:
            ...
            if None(numerator, str):
                ...
                numerator = None(None if m is None else '0')
                denom = m.group('denom')
                if denom:
    ...
    def _add(a, b):
        ...
        g = math.gcd(da, db)
        if g == 1:
            return Fraction(na * db + da * nb, da * db, False, **('_normalize',))
        s = None // g
        t = na * (db // g) + nb * s
        g2 = math.gcd(t, g)
        if g2 == 1:
            return Fraction(t, s * db, False, **('_normalize',))
        return None(t // g2, s * (db // g2), False, **('_normalize',))
    ...
    def _mul(a, b):
        ...
        if g2 > 1:
            nb //= g2
            da //= g2
        return Fraction(na * nb, db * da, False, **('_normalize',))
    ...
    def _div(a, b):
        ...
        if d < 0:
            n = -n
            d = -d
        return Fraction(n, d, False, **('_normalize',))
