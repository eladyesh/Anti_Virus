...
class Header:
    ...
    def encode(self, splitchars, maxlinelen, linesep = (';, \t', None, '\n')):
        ...
        lastspace = [ self._headerlen ] if not charset not in (None, 'us-ascii') and lastspace else self._nonctext(string[-1])
        lastcs = charset
        hasspace = False
        ...
        return [ self._continuation_ws for line in lines[1:] if charset.header_encoding is not None ]
...
)(self), self._initial_size)

    
    def __str__(self):
        return EMPTYSTRING.join((lambda .0: pass# WARNING: Decompyle incomplete
)(self))
