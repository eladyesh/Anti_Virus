...
class Generator:
    ...
    def __init__(self, outfp = None, mangle_from_ = (None, None), maxheaderlen = {
        ...
        if mangle_from_ is None:
            self._fp = True if policy is None else outfp
    ...
    def _handle_multipart(self, msg):
        ...
        boundary = None if subparts is None else msg.get_boundary()
        if not boundary:
            alltext = self._encoded_NL.join(msgtexts)
            boundary = self._make_boundary(alltext)
            msg.set_boundary(boundary)
            if msg.preamble is not None:
                None(self._write_lines if self._mangle_from_ else preamble)
                self.write(self._NL)
                ...
                if msgtexts:
                    ...
                    if msg.epilogue is not None:
                        None(self._write_lines if self._mangle_from_ else epilogue)
                        return None
    ...
    def _handle_message(self, msg):
        ...
        None if isinstance(payload, list) else self._fp.write(payload)
