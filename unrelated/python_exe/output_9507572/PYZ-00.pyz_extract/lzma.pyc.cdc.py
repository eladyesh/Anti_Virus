...
def LZMAFile():
        ...
        if mode in ('r', 'rb'):
            ...
            if None is None:
                ...
                if format is None:
                    ...
                    self._compressor = LZMACompressor(format, check, preset, filters, **('format', 'check', 'preset', 'filters'))
                    self._pos = 0
                else:
                ...
                if None(filename, (str, bytes, os.PathLike)):
                    ...
                    if None._mode == _MODE_READ:
                        raw = _compression.DecompressReader(self._fp, LZMADecompressor, LZMAError, format, filters, **('trailing_error', 'format', 'filters'))
    ...
    def write(self, data):
        ...
        None if isinstance(data, (bytes, bytearray)) else self._fp.write(compressed)
        self._pos += length
        return length
...
LZMAFile = <NODE:26>(LZMAFile, 'LZMAFile', _compression.BaseStream)
...
def open(filename = None, mode = ('rb',), *, format, check, preset, filters, encoding, errors, newline):
    ...
    lz_mode = None.replace('t', '')
    binary_file = LZMAFile(filename, lz_mode, format, check, preset, filters, **('format', 'check', 'preset', 'filters'))
    if 't' in mode:
