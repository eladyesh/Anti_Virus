...
class TextWrapper:
    ...
    for x in _whitespace:
        ...
        otherwise.
        """
            chunks = None if self.break_on_hyphens is True else (lambda .0: [ c for c in .0 if c ])(chunks)
            return chunks
        ...
        '''
            ...
            if None.max_lines is not None and None if self.max_lines > 1 else len(indent) + len(self.placeholder.lstrip()) > self.width:
                raise ValueError('placeholder too large for max width')
            None.reverse()
            if chunks:
                cur_line = []
                cur_len = 0
                width = None - self.width if lines else len(indent)
                ...
                elif chunks and len(chunks[-1]) > width:
                    self._handle_long_word(chunks, cur_line, cur_len, width)
                    cur_len = sum(map(len, cur_line))
                    if self.drop_whitespace and cur_line and cur_line[-1].strip() == '':
                        cur_len -= len(cur_line[-1])
                        del cur_line[-1]
                        if not (cur_line or self.max_lines is None) and len(lines) + 1 < self.max_lines:
